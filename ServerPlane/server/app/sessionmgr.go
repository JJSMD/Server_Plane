...
func init() {
	SessionMgr = sessionMgr{sessionKey: "sessionKey123456", connMap:make(map[string] IFace.IConnection)}
}

type sessionMgr struct {
	sessionKey string
	connMap    map[string] IFace.IConnection
	lock 	   sync.RWMutex
}
...
func (s *sessionMgr) SessionEnter(session string, conn IFace.IConnection)  {
...
func (s *sessionMgr) SessionExitByConn(conn IFace.IConnection)  {

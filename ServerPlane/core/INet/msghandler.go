...
func NewMsgHandle(workerSize uint32) *MsgHandle {
	return &MsgHandle{
		...
		//一个worker对应一个queue
		TaskQueue:make([]chan IFace.IRequest, utils.GlobalObject.ServerWorkerSize),
		...
		if l, ok := mh.Apis[nameSpace]; ok {
			for handler := l.Front(); nil != handler; handler = handler.Next() {
				...
				if method.IsValid() == false {
					...
					isFound = true
					router := handler.Value.(IFace.IRouter)
					//执行对应处理方法
...
//为消息添加具体的处理逻辑
func (mh *MsgHandle) AddRouter(router IFace.IRouter) {
...
//启动一个Worker工作流程
func (mh *MsgHandle) StartOneWorker(workerID int, taskQueue chan IFace.IRequest,taskExit chan bool) {

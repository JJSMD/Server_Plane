...
//创建连接的方法
func NewConnection(netWork IFace.INetWork, conn *net.TCPConn, connID uint32, msgHandler IFace.IMsgHandle) *Connection {
	//初始化Conn属性
	c := &Connection{
		TcpNetWork:   netWork,
		Conn:         conn,
...
func (c *Connection) GetTcpNetWork() IFace.INetWork {
	return c.TcpNetWork
}

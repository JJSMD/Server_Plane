...
func TestTimerWheel(t *testing.T) {
	...
	//给时间轮添加定时器
	timer1 := NewTimerAfter(10 * time.Second, myFunc, []interface{}{1,10})
	_ = hour_tw.AddTimer(1, timer1)
	fmt.Println("add timer 1 done!")

	//给时间轮添加定时器
	timer2 := NewTimerAfter(20 * time.Second, myFunc, []interface{}{2,20})
	_ = hour_tw.AddTimer(2, timer2)
	fmt.Println("add timer 2 done!")

	//给时间轮添加定时器
	timer3 := NewTimerAfter(30 * time.Second, myFunc, []interface{}{3,30})
	_ = hour_tw.AddTimer(3, timer3)
	fmt.Println("add timer 3 done!")

	//给时间轮添加定时器
	timer4 := NewTimerAfter(40 * time.Second, myFunc, []interface{}{4,40})
	_ = hour_tw.AddTimer(4, timer4)
	fmt.Println("add timer 4 done!")

	//给时间轮添加定时器
	timer5 := NewTimerAfter(50 * time.Second, myFunc, []interface{}{5,50})
